"""One owner for 39 jobs, two budget roots, receipts, solutions and safeguards."""
import boot
from util import *
...
class Coordinator:
    ...
    def worker(self, job):
        return boot.HERE / "pc_worker.py" if job["variant"] == "PC" else boot.FROZEN / "worker.py"
    ...
    def status(self, pending, active, force=False):
        ...
        value = {"status": "RUNNING" if not self.stop_reason else "PAUSING", "utc": self.host.last["utc"],
                 ...
                 "budget_eta_seconds": remain / rate if rate else None,
                 "eta_scope": "remaining computational budget, not solution time",
