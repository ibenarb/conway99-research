...
def check(run):
    ...
    try:
        ...
        if len(os.sched_getaffinity(0)) < 18:
            raise RuntimeError("Fewer than 18 available logical CPUs")
        ...
        result = {"status": "READY_NOT_STARTED", "fingerprint_sha256": file_sha(run / "FINGERPRINT.json"),
                  ...
                  "host": host.sample(), "jobs": 39, "budget_cpu_hours": 121}
...
def do_run(run):
    ...
    try:
        ...
        if state == "COMPLETE":
            ...
            # Fixed record starters are unaffected by this later read-only harvest.
            harvest_path = run / "HARVEST_SUMMARY.json"
            if not harvest_path.exists() and ledger.remaining("harvest") >= 10:
                summary = auxiliary(run, host, ledger, "harvest", "harvest", harvest_path, (run,))
                print("HARVEST " + json.dumps(summary), flush=True)
            if (run / "SOLUTION.json").exists():
                candidate = read(run / "SOLUTION.json")["candidate"]
                if checked(candidate["graph6"], "lambda")[1]["F"] != 0:
                    raise RuntimeError("Invalid harvest solution marker")
                state = "SOLUTION"
            print("FOLLOWUP_VERIFIED " + json.dumps(report), flush=True)
            atomic(run / "COMPLETE.json", {"status": "FOLLOWUP_VERIFIED", "utc": host.sample()["utc"]})
        else:
...
def main():
    ...
    if args.action == "prepare":
        source = (args.source or Path.home() / "conway99_workspace/ryzen_lambda_compare_020_20260921").expanduser().resolve()
        prepare(source, run)
        return
