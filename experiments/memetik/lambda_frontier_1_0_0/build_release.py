...
def main():
    ...
    package = {'version': 'lambda-frontier-1.0.0', 'search_cpu_hours': 32, 'maximum_total_cpu_hours': 33, 'jobs': 8,
    ...
    dest = ROOT / 'releases/memetik/lambda_frontier_1_0_0.zip'
    start = '''from pathlib import Path
import subprocess
import sys
root = Path(__file__).resolve().parent
cli = root / "experiments/memetik/lambda_frontier_1_0_0/run.py"
run = Path.home() / "conway99_workspace/ryzen_lambda_frontier_100_20260924"
if run.exists():
...
'''
    with zipfile.ZipFile(dest, 'w', zipfile.ZIP_DEFLATED) as z:
        for name in names:
            z.write(ROOT / name, 'lambda_frontier_1_0_0/' + name)
        z.writestr('lambda_frontier_1_0_0/start.py', start)
