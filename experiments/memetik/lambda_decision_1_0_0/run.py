...
def check(run):
    ...
    try:
        ...
        result = {"status": "READY_NOT_STARTED", "fingerprint_sha256": file_sha(run / "FINGERPRINT.json"),
                  ...
                  "host": host.sample(), "jobs": 14, "search_budget_cpu_hours": 60, "maximum_total_cpu_hours": 62}
...
def do_run(run):
    ...
    try:
        ...
        if state == "COMPLETE":
            ...
            print("DECISION_VERIFIED " + json.dumps(report), flush=True)
            atomic(run / "COMPLETE.json", {"status": "DECISION_VERIFIED", "utc": host.sample()["utc"]})
        else:
