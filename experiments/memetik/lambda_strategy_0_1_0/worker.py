...
from strategy_search import (Guard, candidate, episode, fresh_costs, identity, key,
...
def stop_requested():
    return STOP or os.getppid() != PARENT or (len(sys.argv) > 1 and (Path(sys.argv[1]).parent.parent / 'SOLUTION.json').exists())
...
def main():
    ...
    if task['kind'] == 'controls':
        import unittest
        import controls
        resource.setrlimit(resource.RLIMIT_CPU, (598, 600))
        tests = unittest.TextTestRunner().run(unittest.defaultTestLoader.loadTestsFromTestCase(controls.Controls))
        if not tests.wasSuccessful():
            raise RuntimeError('Controls failed')
        atomic(directory / 'result.json', {'status':'CONTROLS_PASS','tests':tests.testsRun,'real':controls.real_report()})
        return
    ...
    from strategy import CycleSource, quota_parent, quota_select
    global parent_choice, select
    if task['variant'] in ('CYCLE', 'CYCLE_QUOTA'):
        strategy_search.MoveSource = CycleSource
    if task['variant'] == 'CYCLE_QUOTA':
        parent_choice, select = quota_parent, quota_select
    config = task["config"]
    ...
    observer = Observer(directory, state, deadline, base, task)
    strategy_search.OBSERVER = observer.observe
    last_save = 0.0
    ...
    while not stop_requested() and cpu() < deadline:
        ...
        state.setdefault('parent_families', {})[parent['family']] = state.setdefault('parent_families', {}).get(parent['family'], 0) + 1
        result, record, memory = episode(parent, arm, target, "A0", rng, config,
                                         state["memory"].get(line, []), state["failures"].get(line, 0),
                                         deadline, stop_requested, best)
        observer.record_episode(record, result)
        state["episodes"] += 1
        ...
        if record["eligible_before_endpoint"]:
            state["valid_completed_episodes"] = state.get("valid_completed_episodes", 0) + 1
    ...
    atomic(directory / "result.json", {"status": state["status"], "best": state["best"],
                                      ...
                                      "end_class_hashes": sorted(seen_classes),
                                      "valid_completed_episodes": state.get("valid_completed_episodes", 0),
                                      ...
                                      "parent_families": state.get("parent_families", {}),
