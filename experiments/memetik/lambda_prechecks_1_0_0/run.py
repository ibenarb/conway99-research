...
def check(run):
    ...
    try:
        ...
        result = {"status": "READY_NOT_STARTED", "fingerprint_sha256": file_sha(run / "FINGERPRINT.json"),
                  ...
                  "host": host.sample(), "jobs": 24, "budget_cpu_hours": 34}
...
def do_run(run):
    ...
    try:
        ...
        if state == "COMPLETE":
            ...
            print("PRECHECKS_VERIFIED " + json.dumps(report), flush=True)
            atomic(run / "COMPLETE.json", {"status": "PRECHECKS_VERIFIED", "utc": host.sample()["utc"]})
        else:
