...
def main():
    ...
    package = {'version': 'lambda-prechecks-1.0.0', 'cpu_hours': 34, 'jobs': 24,
    ...
    dest = ROOT / 'releases/memetik/lambda_prechecks_1_0_0.zip'
    start = '''from pathlib import Path
import subprocess
import sys
root = Path(__file__).resolve().parent
cli = root / "experiments/memetik/lambda_prechecks_1_0_0/run.py"
run = Path.home() / "conway99_workspace/ryzen_lambda_prechecks_100_20260924"
if run.exists():
...
'''
    with zipfile.ZipFile(dest, 'w', zipfile.ZIP_DEFLATED) as z:
        for name in names:
            z.write(ROOT / name, 'lambda_prechecks_1_0_0/' + name)
        z.writestr('lambda_prechecks_1_0_0/start.py', start)
