...
def run_checks(run):
    ...
    for job in plan['jobs']:
        ...
        seeds.add(task['seed'])
    if [j['frontier_W'] for j in plan['jobs']] != [2081] * 6 + [2092] * 2:
        raise RuntimeError('Changed 6:2 allocation')
    if len(seeds) != 8:
        raise RuntimeError('Repeated random seed')
    overlap = len({p['class'] for p in banks['2081']['population']}
                  & {p['class'] for p in banks['2092']['population']})
    if overlap != 0:
