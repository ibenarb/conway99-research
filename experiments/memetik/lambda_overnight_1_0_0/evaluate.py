...
def evaluate(run):
    ...
    for job in plan['jobs']:
        ...
        for c in [*candidates, *result['curves'], *result['milestones'].values()]:
        ...
        rows.append({'id': job['id'], 'seed': job['seed'], 'start_W': job['frontier_W'],
                     ...
                     'milestones': {k: {'scores': c['scores'], 'observed_cpu': c['observed_cpu']}
                                    for k, c in result['milestones'].items()},
    ...
    if cpu_total > 201600 or aux > 3600:
