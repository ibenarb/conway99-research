...
def specifications():
    return [{'id': f'F-W{w}-{rep}', 'group': f'F{w}', 'variant': 'P', 'target': 'W',
             'frontier_W': w, 'replicate': rep, 'seed': 2026092800 + (0 if i == 0 else 6) + rep,
             'cumulative_budget_seconds': 25200, 'additional_cpu_seconds': 25200}
            for i, w in enumerate((2081, 2092)) for rep in range(6 if i == 0 else 2)]
...
def prepare(source, run):
    ...
    try:
        ...
        for spec in specifications():
            ...
            cfg['milestones_cpu_seconds'] = [3600, 7200, 14400, 21600, 25200]
            task = {**spec, 'kind': 'compare', 'arm': 'lambda', 'worker_cpu_seconds': 25200,
        ...
        atomic(run / 'runs/comparison/budget.json', {'per_job_cpu_seconds': 25200})
        plan = {'version': 'lambda-overnight-1.0.0', 'workers': 8, 'jobs': jobs,
                'queue': [j['id'] for j in jobs], 'search_additional_cpu_seconds': 201600,
                'authorized_search_cpu_hours': 56, 'maximum_aux_cpu_hours': 1,
                'automatic_extension': False, 'primary_threshold': 2081,
    ...
    print(json.dumps({'status': 'PREPARED_NOT_STARTED', 'run': str(run), 'jobs': 8,
                      'additional_search_cpu_hours': 56, 'maximum_aux_cpu_hours': 1}), flush=True)
...
def verify(run):
    ...
    if (len(plan['jobs']) != 8 or plan['workers'] != 8
            or sum(j['additional_cpu_seconds'] for j in plan['jobs']) != 201600
