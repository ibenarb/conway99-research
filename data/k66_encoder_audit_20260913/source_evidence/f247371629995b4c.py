...
def lrat_verified(path: Path) -> bool:
    return b'c VERIFIED' in path.read_bytes()


def tool_controls(run: Path, tools: dict[str, str], guard: Safety, heap_mb: int) -> None:
    directory = run / ('controls_' + stamp())
    directory.mkdir()
    unsat = directory / 'unsat.cnf'
    proof = directory / 'proof.lrat'
    sat_cross = directory / 'sat_cross.cnf'
    sat_bad = directory / 'sat_bad.cnf'
    bad_proof = directory / 'bad_proof.lrat'

    # Positive end-to-end control: CaDiCaL must prove a tiny contradiction and both
    # checkers must emit their explicit positive verdicts.
    ...
    require(lrat['exit'] == 0 and lrat_verified(directory / 'lrat_positive.out'),
            'Positive LRAT control failed or explicit VERIFIED marker absent')
    require(cake['exit'] == 0 and marker_present(directory / 'cake_positive.out'),
            'Positive Cake control failed')

    # The historical C lrat-check has a known permissive edge case for an empty
    # lemma whose unit hints never reach a conflict.  Therefore a cross-formula
    # test is diagnostic only for lrat-check; Cake remains the sound acceptance
    # gate.  Cake must reject the valid UNSAT proof when paired with this SAT CNF.
    sat_cross.write_text('p cnf 1 2\n1 0\n1 0\n')
    lrat_cross = guard.execute([tools['lrat-check'], str(sat_cross), str(proof)], directory, 'lrat_cross_formula')
    cake_cross = guard.execute(cake_command(tools, sat_cross, proof, heap_mb), directory, 'cake_cross_formula')
    require(not (cake_cross['exit'] == 0 and marker_present(directory / 'cake_cross_formula.out')),
            'Cake accepted an UNSAT proof against a satisfiable cross-formula control')

    # Independent lrat-check sanity control: the first hinted clause has two
    # unassigned literals, so this explicit empty-clause proof must be rejected
    # even by the permissive historical checker.  This tests that the binary is
    # not blindly returning success; it is not used as a soundness theorem.
    sat_bad.write_text('p cnf 2 1\n1 2 0\n')
    bad_proof.write_text('2 0 1 0\n')
    lrat_bad = guard.execute([tools['lrat-check'], str(sat_bad), str(bad_proof)], directory, 'lrat_negative_sanity')
    cake_bad = guard.execute(cake_command(tools, sat_bad, bad_proof, heap_mb), directory, 'cake_negative_sanity')
    require(lrat_bad['exit'] != 0 and not lrat_verified(directory / 'lrat_negative_sanity.out'),
            'LRAT checker failed the explicit invalid-proof sanity control')
    require(not (cake_bad['exit'] == 0 and marker_present(directory / 'cake_negative_sanity.out')),
            'Cake accepted the explicit invalid-proof sanity control')

    cross_out = directory / 'lrat_cross_formula.out'
    save(directory / 'controls.json', {
        'status': 'PASS',
        'positive_lrat_verified_marker': True,
        'positive_cake_verified_marker': True,
        'cake_cross_formula_rejected': True,
        'lrat_invalid_proof_sanity_rejected': True,
        'cake_invalid_proof_sanity_rejected': True,
        'legacy_lrat_cross_formula_exit': lrat_cross['exit'],
        'legacy_lrat_cross_formula_verified_marker': lrat_verified(cross_out),
        'legacy_lrat_cross_formula_hint_satisfied_warning': b'WARNING hint already satisfied' in cross_out.read_bytes(),
        'lrat_negative_scope': 'sanity-only; Cake positive verdict is the formal acceptance gate',
        'proof_sha256': sha(proof),
    })
    print('TOOLCHAIN_CONTROLS_PASS positive_chain Cake_wrong_formula_reject LRAT_invalid_proof_reject', flush=True)
...
def completed_certificate(run: Path, job: Any, manifest_sha: str) -> dict[str, Any] | None:
    ...
    require(record['lrat_exit'] == record['cake_exit'] == 0 and record.get('lrat_verified_marker') is True and record['cake_positive_marker'] is True,
    ...
    require(lrat_verified(run / record['lrat_stdout']), 'Stored LRAT VERIFIED marker absent')
...
def run_job(run: Path, job: Any, tools: Any, guard: Safety, checker_lock: threading.Lock,
    ...
    try:
        ...
        lrat_marker = lrat_verified(checks / 'lrat.out')
        require(lrat['exit'] == 0 and lrat_marker, f'{job["id"]}: LRAT rejection or VERIFIED marker absent')
        ...
        record = {'id': job['id'], 'status': 'UNSAT_CERTIFIED', 'manifest_sha256': manifest_sha,
                  ...
                  'lrat_exit': 0, 'lrat_verified_marker': True, 'cake_exit': 0, 'cake_positive_marker': True,
