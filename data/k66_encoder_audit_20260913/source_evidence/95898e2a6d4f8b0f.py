...
import argparse, concurrent.futures as cf, gzip, hashlib, json, os, shutil, signal, subprocess, threading, time, zipfile
...
def main():
    ...
    for rid in ROOT_IDS:
        ...
        roots[rid]=str(p); ranks[rid]=primary_occurrence_order(p,PRIMARY_COUNTS[rid])
    ...
    manifest={"format":"CONWAY99-K66-DEEP7-COARSECERT-1","roots":roots,"root_hashes":ROOT_HASHES,
              "old_tree_state_sha256":sha(oldrun/"state.json"),"cut_depth":a.cut_depth,
    ...
    def do_split(c):
        ...
        for ch in children:
            if ch["id"] not in st["cubes"]:
                st["cubes"][ch["id"]]={"id":ch["id"],"root_id":ch["root_id"],"lits":list(ch["lits"]),
    ...
    print(f"initial_frontier={len(st.get('initial_frontier',[]))} cut_depth={a.cut_depth} workers={workers}",flush=True)
    ...
    try:
        while True:
            ...
            pending=[c for c in st["cubes"].values() if c["status"]=="PENDING"]
            while pending and len(solver_futs)<workers and free>=max(a.disk_floor_gib+workers*a.proof_cap_gib,150):
        ...
        summary={"format":"CONWAY99-K66-DEEP7-COARSECERT-SUMMARY-1","roots":ROOT_IDS,
                 ...
                 "claim_scope":"Seven reduced profile-conflict root CNFs are fully covered by exact binary split trees; every terminal leaf is LRAT+Cake certified UNSAT. Proofs are stored losslessly gzip-compressed after verification."}
        atomic_json(run/"summary.json",summary)
        hp,win=make_handoff(run,st)
        print("K66_DEEP7_COARSECERT_DONE",json.dumps(summary,sort_keys=True),flush=True)
        print("HANDOFF",hp,flush=True)
        if win: print("HANDOFF_WINDOWS",win,flush=True)
        print("HANDOFF_SHA256",sha(hp),flush=True)
