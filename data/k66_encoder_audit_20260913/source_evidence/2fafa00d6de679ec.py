...
def solve(job,cnf,cadical,seconds):
    ...
    with open(out/"solver.out","wb") as o,open(out/"solver.err","wb") as e:
        t=time.time(); rc=subprocess.run([cadical,"--plain",f"-t{int(seconds)}","-w",str(model),str(cnf)],stdout=o,stderr=e).returncode
...
def main():
    ...
    outdir=run/"profile_conflict_cnf_scout_20260909"; outdir.mkdir(exist_ok=True)
    data={}; meta={}; jobs=[]
