...
class Budget:
    ...
    def check(self):
        if time.monotonic() - self.last >= 10:
            ...
            disks = [self.directory]
            if Path('/mnt/c').is_dir():
                disks.append(Path('/mnt/c'))
            if any(shutil.disk_usage(disk).free < 10 * 2**30 for disk in disks):
...
def connection(path):
    ...
    db.execute('CREATE INDEX IF NOT EXISTS pending_nodes ON nodes(expanded,id)')
    db.commit()
    return db
...
def execute(task, founder, directory, limits):
    ...
    committed_count = db.execute('SELECT COUNT(*) FROM nodes').fetchone()[0]
    committed_expanded = db.execute('SELECT COUNT(*) FROM nodes WHERE expanded=1').fetchone()[0]
    def save(cpu):
        result.update(discovered_committed=committed_count, expanded_committed=committed_expanded, cpu_seconds=cpu, utc=time.strftime('%Y-%m-%dT%H:%M:%SZ', time.gmtime()))
        atomic(output, result)
    ...
    try:
        while True:
            ...
            for family, move, child in neighbors(rows, founder['arm'], budget, census):
                ...
                if not exists:
                    if node_count >= limits['max_states']:
            ...
            db.commit()
            committed_count = db.execute('SELECT COUNT(*) FROM nodes').fetchone()[0] if task['mode'] == 'descent' else node_count
            committed_expanded += 1
