...
def controller(args):
    ...
    try:
        ...
        save(out/'config.json', {'version': VERSION, 'source': str(HERE),
             ...
             'scope':'Matching orbit search. Every original solution has a frame-equivalent representative in the 11 cases. UNSAT requires certificates.'})
        ...
        while queue or active:
            ...
            snapshot = dict(safety, phase='MATCHING_SEARCH', utc=datetime.datetime.now(datetime.timezone.utc).isoformat(),
        ...
        if status!='GRAPH_FOUND_REQUIRES_REVIEW':
            status='MATCHING_SEARCH_COMPLETE_NO_CERTIFICATION'
...
def main():
    ...
    for _ in range(30):
        if (out/'ready.json').exists():
            print(json.dumps({'status':'C2_MATCHING_READY','version':VERSION,'pid':proc.pid,'output':str(out),'proof_logging':False}),flush=True)
            return
        if proc.poll() is not None:
