...
def check():
    ...
    jobs = [dict(id=f'{g}__tiny_{i}', group=g, seed=0, cnf=str(cnf),
                 cnf_sha256=sha(cnf), assumptions=[]) for g in ('reference', 'totalizer') for i in range(11)]
    ...
    assert report['status'] == 'COUNTER_AB_COMPLETE_NO_CERTIFICATION', report
    assert len(report['results']) == 22
    assert all(r['status'] == 'UNSAT_UNCERTIFIED' for r in report['results'])
    latest_reference = max((out / j['id'] / 'result.json').stat().st_mtime_ns for j in jobs[:11])
    earliest_variant = min((out / j['id'] / 'job.json').stat().st_mtime_ns for j in jobs[11:])
    assert latest_reference <= earliest_variant
    assert not list(root.rglob('*.lrat'))
    return dict(status='PASS', completed=22, wave_barrier_checked=True,
