...
@{distribution=$d;drive=$letter;free_bytes=$v.AvailableFreeSpace;temp=$env:TEMP;utc=[DateTime]::UtcNow.ToString('o')} | ConvertTo-Json -Compress
"""
...
function State($obj) {
    ...
    Move-Item -LiteralPath (Join-Path $dir 'state.tmp') -Destination (Join-Path $dir 'state.json') -Force
}
...
class Guard:
    def __init__(self, out, floor_gib=50):
        ...
        self.windows_dir = self.info['temp'].rstrip('\\') + '\\Conway99_guard_' + uuid.uuid4().hex
    ...
    def read(self):
        state = json.loads((self.directory/'state.json').read_text(encoding='utf-8-sig'))
        if state.get('stop_reason'):
            raise RuntimeError(str(state['stop_reason']))
        if abs(time.time()-state['unix']) > 30:
            raise RuntimeError('Windows guard telemetry stale; stopping')
        return state
