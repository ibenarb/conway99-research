#!/usr/bin/env python3
"""Matched-seed A/B scout runner for Conway_99 O3-BREADTH-1.

Design:
- 139 C4-free types, each solved with layers A and B on one pinned logical CPU.
- A and B of the same type use exactly the same deterministic CaDiCaL seed.
- Default 20 workers on the first 10 physical cores using both SMT siblings.
- The final 2 physical cores are reserved for the OS/controller.
- Default CaDiCaL limit is 5760 s per layer, chosen after the 2026-09-04
  10-vs-20 SMT crossover benchmark to approximate the search work of the
  earlier 3600 s / one-thread-per-physical-core condition.
...
- A/B soundness inconsistency causes hard stop.
...
def instance_seed(master_seed, tau, part):
    """Deterministic seed shared by A and B for one quotient type."""
    raw = ("%d|%d|%s" % (master_seed, tau, ",".join(map(str, part)))).encode()
...
def discover_cpu_groups():
    """Return SMT sibling groups ordered by package/core."""
    ...
    for cpu_dir in sorted(Path("/sys/devices/system/cpu").glob("cpu[0-9]*")):
        ...
        try:
            ...
            if online_path.exists() and online_path.read_text().strip() == "0":
                continue
    ...
    if groups:
        return [
            sorted(groups[key])
            for key in sorted(groups, key=lambda x: (int(x[0]), int(x[1])))
        ]
    n = os.cpu_count() or 1
    return [[i] for i in range(n)]


def select_worker_cpus(cpu_groups, workers, reserve_cores):
    """Use primary threads first, then SMT siblings; reserve whole physical cores."""
    if reserve_cores < 0:
        raise ValueError("reserve_cores must be >= 0")
    if reserve_cores >= len(cpu_groups):
        raise RuntimeError(
            "reserve_cores=%d leaves no usable physical cores out of %d"
            % (reserve_cores, len(cpu_groups))
        )
    usable = cpu_groups[:-reserve_cores] if reserve_cores else list(cpu_groups)
    reserved = cpu_groups[-reserve_cores:] if reserve_cores else []
    max_siblings = max(len(g) for g in usable)
    ordered = []
    for sibling_index in range(max_siblings):
        for group in usable:
            if sibling_index < len(group):
                ordered.append(group[sibling_index])
    if workers > len(ordered):
        raise RuntimeError(
            "need %d logical worker CPUs after reserving %d physical cores; have %d"
            % (workers, reserve_cores, len(ordered))
        )
    return {
        "physical_cpu_groups": [list(g) for g in cpu_groups],
        "physical_cpu_representatives": [min(g) for g in cpu_groups],
        "usable_cpu_groups": [list(g) for g in usable],
        "reserved_cpu_groups": [list(g) for g in reserved],
        "worker_cpus": ordered[:workers],
        "cpu_policy": "primary_threads_then_smt_siblings",
    }
...
def worker_run(cfg, tau, part, cpu):
    ...
    try:
        for layer in LAYERS:
            ...
            seed = instance_seed(cfg["master_seed"], tau, part)
            command = [
                cfg["solver"],
                "-t", str(cfg["timeout"]),
                "--seed=%d" % seed,
            ...
            state["complete"] = len({x["layer"] for x in state["instances"]}) == len(LAYERS)
        ...
        state["complete"] = len({x["layer"] for x in state.get("instances", [])}) == len(LAYERS)
        state["updated_at"] = now_iso()
        atomic_json(type_path, state)
...
def make_manifest(cfg, generic, cpu_layout):
    ...
    manifest = {
        ...
        "physical_cpu_groups": cpu_layout["physical_cpu_groups"],
        "physical_cpu_representatives": cpu_layout["physical_cpu_representatives"],
        "usable_cpu_groups": cpu_layout["usable_cpu_groups"],
        "reserved_cpu_groups": cpu_layout["reserved_cpu_groups"],
        "worker_cpus": cpu_layout["worker_cpus"],
        "cpu_policy": cpu_layout["cpu_policy"],
        "layers": list(LAYERS),
        "experiment_design": {
            "paired_seed": True,
            "seed_scope": "master_seed|tau|cycle_type; identical for A and B",
            "layer_order": "A_then_B",
            "reference_single_thread_budget_seconds": 3600,
            "smt_compensated_timeout_seconds": cfg["timeout"],
            "smt_benchmark_date": "2026-09-04",
            "smt_panel_sha256": "c7ea3d618c056120dfeea303017e261485952887729c7057f1c989e9c2650839",
            "smt_manifest_sha256": "86bbc2d7bd2607aa1d18c945ddcef74a8506dfa367f1ddde2661a253b706dc04",
            "smt_benchmark_runner_sha256": "5a8fb15bdd5a8570fceb1812eab5d122a63e070228293a3c7f692a4cd62261f3",
            "measured_gain_conflicts_per_wall_s": 0.23624601919825827,
            "measured_gain_decisions_per_wall_s": 0.2988857046411135,
            "measured_gain_propagations_per_wall_s": 0.24610532924926587,
        },
        "types": [
            {"ordinal": i, "tau": tau, "cycle_type": list(part), "job_id": job_id(tau, part)}
            for i, (tau, part) in enumerate(types)
        ],
        "instances": len(types) * len(LAYERS),
        "paths": {
...
def validate_environment(cfg, manifest):
    ...
    if len(manifest.get("worker_cpus", [])) < cfg["workers"]:
        problems.append(
            "need %d worker CPUs after reservation, manifest has %d"
            % (cfg["workers"], len(manifest.get("worker_cpus", [])))
        )
    reserved = {
        cpu
        for group in manifest.get("reserved_cpu_groups", [])
        for cpu in group
    }
    overlap = reserved.intersection(manifest.get("worker_cpus", []))
    if overlap:
        problems.append("worker/reserved CPU overlap: %s" % sorted(overlap))
...
def main():
    ...
    ap.add_argument("--timeout", type=int, default=5760)
    ...
    cpu_groups = discover_cpu_groups()
    cpu_layout = select_worker_cpus(
        cpu_groups, cfg["workers"], cfg["reserve_cores"]
    )
    manifest_path = run_dir / "manifest.json"
    if manifest_path.exists():
        manifest = load_json(manifest_path)
        expected = make_manifest(cfg, generic, cpu_layout)
        keys = (
            "schema", "master_seed", "timeout", "workers", "layers", "types", "instances",
            "worker_cpus", "reserved_cpu_groups", "cpu_policy",
            "experiment_design", "paths", "sha256",
        )
        for key in keys:
            if manifest.get(key) != expected.get(key):
                raise RuntimeError("manifest mismatch on resume: %s" % key)
    else:
        manifest = make_manifest(cfg, generic, cpu_layout)
        atomic_json(manifest_path, manifest)

    validate_environment(cfg, manifest)
    print("O3_AB_MATCHED_SCOUT_PREFLIGHT PASS", flush=True)
    print("TYPES=%d INSTANCES=%d WORKERS=%d TIMEOUT=%ds" % (
        len(manifest["types"]), manifest["instances"], cfg["workers"], cfg["timeout"]
    ), flush=True)
    print("WORKER_CPUS=%s RESERVED_PHYSICAL_CORES=%d RESERVED_CPU_GROUPS=%s" % (
        manifest["worker_cpus"], cfg["reserve_cores"], manifest["reserved_cpu_groups"]
    ), flush=True)
    print("MATCHED_SEEDS=True SEED_SCOPE=TYPE A_B_ORDER=A_then_B", flush=True)
