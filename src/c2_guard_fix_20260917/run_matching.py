...
def controller(args):
    ...
    try:
        ...
        save(out/'config.json', {'version': VERSION, 'source': str(HERE),
             ...
             'scope':'Single wave: eleven totalizer matching cases, bounded 18-hour scout; no certification.'})
...
def main():
    ...
    parser.add_argument('--seconds',type=int,default=64800,help='wall seconds per matching case; default and maximum 64800 (18 hours)')
    parser.add_argument('--workers',type=int,default=11)
    parser.add_argument('--controller',action='store_true',help=argparse.SUPPRESS)
    parser.add_argument('--out',help=argparse.SUPPRESS)
    args=parser.parse_args()
    if not 3<=args.seconds<=64800 or not 1<=args.workers<=11:
        parser.error('seconds: 3..64800; workers: 1..11')
    if args.controller:
        controller(args)
        return
