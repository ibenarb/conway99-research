...
function ReadHeartbeat {
    ...
    try {
        ...
        $stream=[IO.File]::Open((Join-Path $dir 'heartbeat'),[IO.FileMode]::Open,[IO.FileAccess]::Read,$share)
        $reader=[IO.StreamReader]::new($stream)
        return [long]($reader.ReadToEnd())
    } finally {
...
function State($obj) {
    ...
    for($attempt=0; $attempt -lt 40; $attempt++) {
        try {
            if([IO.File]::Exists($dst)){[IO.File]::Replace($src,$dst,(Join-Path $dir 'state.previous.json'))}
...
class Guard:
    ...
    def beat(self):
        ...
        while True:
            try:
                # Refresh timestamp on each attempt; old published heartbeat survives failure.
                ...
                if time.monotonic() >= deadline:
                    raise RuntimeError(
                        f'WINDOWS_HEARTBEAT_WRITE_FAILED after 5s: {temporary} -> '
                        f'{self.directory / "heartbeat"}; retries={self.write_retries}'
                    ) from exc
    ...
    def read(self):
        if hasattr(self, 'proc') and self.proc.poll() is not None:
            draft = self.directory/'state.tmp'
            try:
                detail = draft.read_text(encoding='utf-8-sig')[:4096]
            except OSError as exc:
                detail = str(exc)
            raise RuntimeError(f'WINDOWS_OBSERVER_EXITED code={self.proc.returncode} directory={self.directory} draft={detail}')
