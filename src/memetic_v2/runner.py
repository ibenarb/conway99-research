...
from core import objective_key, metrics
...
REFERENCE = REPOSITORY / "data/memetic_v2/reference"
...
def assert_candidate(candidate):
    ...
    hist = report["residual_histogram"]
    expected = {"L1": sum(abs(int(r))*n for r,n in hist.items()),
                "Linf": max(abs(int(r)) for r in hist),
                "Nmax": sum(n for r,n in hist.items() if abs(int(r)) == max(abs(int(x)) for x in hist))}
    if any(candidate.get(k) != v for k,v in expected.items()):
        raise RuntimeError("Independent norm verification failed")
...
def reference_candidates():
    source = json.loads((REFERENCE / "population64.json").read_text())
    originals = source["candidates"]
    if len(originals) != 64 or len({p["canonical"] for p in originals}) != 64:
        raise RuntimeError("Exactly 64 nonisomorphic starters required")
    if Counter(p["arm"] for p in originals) != {"omega": 32, "lambda": 32}:
        raise RuntimeError("Expected 32 starters per arm")
    if not {"A_legacy_best_hard", "B_maple_20260829"} <= {p["founder"] for p in originals}:
        raise RuntimeError("Historical founders A and B are mandatory")
    result = []
    for original in originals:
        candidate = dict(original)
        candidate.update(metrics(decode_g6(candidate["g6"])))
...
def update_archive(state, candidate, config):
    ...
    if any(c["canonical"] == candidate["canonical"] and c["objective"] == candidate["objective"] for c in archive):
        return
    archive.append(candidate)
    if len(archive) <= config["archive_cells"]:
        return
    protected = set()
    groups = {(p["objective"], p["arm"], p["founder"]) for p in archive}
    for group in groups:
        indices = [i for i,p in enumerate(archive) if (p["objective"],p["arm"],p["founder"]) == group]
        for metric in ("W", "L1", "F", "Linf"):
            protected.add(min(indices, key=lambda i: archive[i][metric]))
    removable = [i for i in range(len(archive)) if i not in protected]
    if not removable:
        raise RuntimeError("Archive capacity too small for protected records")
    # Oldest unprotected state; all four per-founder records remain available.
    del archive[removable[0]]
...
def build_generation_jobs(state, config, calibration=False):
    ...
    if calibration:
        population = [p for objective in config["objectives"] for arm in ("omega", "lambda") for p in [q for q in population if q["arm"] == arm and q["objective"] == objective][:2]]
    ...
    for parent in population:
        ...
        for ordinal, (kind, length) in enumerate(specifications):
            ...
            partners = [p for p in population if p["arm"] == parent["arm"] and p["objective"] == parent["objective"] and p["canonical"] != parent["canonical"]]
            other_families = [p for p in partners if p["founder"] != parent["founder"]]
    ...
    by_objective = [[j for j in jobs if j["parent"]["objective"] == objective] for objective in config["objectives"]]
    return [job for group in zip(*by_objective) for job in group]
...
def select_generation(state, config):
    ...
    for position, parent in enumerate(list(population)):
        others = [p for i, p in enumerate(population) if i != position and p["objective"] == parent["objective"]]
        rank = lambda p: objective_key(p, parent["objective"])
        identities = {p["canonical"] for p in others}
        acceptable = [p for p in grouped[parent["line_id"]] if rank(p) <= rank(parent) and p["canonical"] != parent["canonical"] and p["canonical"] not in identities]
        if acceptable:
            best_score = min(rank(p) for p in acceptable)
            ties = [p for p in acceptable if rank(p) == best_score]
            ...
            child["stagnant_generations"] = 0 if rank(child) < rank(parent) else parent["stagnant_generations"] + 1
...
class Coordinator:
        ...
        if self.state is None:
            ...
            for objective in config["objectives"]:
                for original in originals:
                    ...
                    candidate["line_id"] = objective + "-" + original["line_id"]
                    references.append(candidate)
            self.state = {"phase": "CALIBRATION", "generation": 0, "config": config,
                          "fingerprint": current_fingerprint, "population": references,
                          "pool": [], "archive": [], "results": {},
                          "jobs": [], "completed_tasks": 0,
                          "startup_active_seconds": 0.0, "pilot_active_seconds": 0.0,
                          "recent_task_wall": [], "cumulative_cpu_seconds": 0.0,
                          "created_utc": datetime.now(timezone.utc).isoformat()}
            self.state["jobs"] = build_generation_jobs(self.state, config, calibration=True)
    ...
    def launch(self, task):
        ...
        task = dict(task)
        if "parent" in task:
            task["novelty_reference"] = [{"defect_degrees": q["defect_degrees"], "residual_histogram": q["residual_histogram"]} for q in self.state["population"] if q["line_id"] != task["parent"]["line_id"] and q["objective"] == task["parent"]["objective"]]
        ...
        process = subprocess.Popen([sys.executable, str(REPOSITORY / "src/memetic_v2/worker.py"), "--input", str(input_path), "--output", str(output_path)],
    ...
    def accept_result(self, task, result):
        ...
        objective = task["parent"]["objective"]
        work = self.state.setdefault("variant_work", {}).setdefault(objective, {"tasks": 0, "cpu_seconds": 0.0})
        work["tasks"] += 1
        work["cpu_seconds"] += result.get("cpu_seconds", 0)
        ...
        self.tick()
        elapsed = self.state["startup_active_seconds"] + self.state["pilot_active_seconds"]
        samples = self.state.setdefault("throughput_samples", [])
        samples.append([elapsed, self.state["completed_tasks"]])
        self.state["throughput_samples"] = samples[-128:]
        self.checkpoints.save(self.state)
        ...
        try:
            while not self.stopping:
                ...
                if now - self.last_status >= self.config["status_seconds"]:
                    ...
                    write_report(self.directory, self.state, report)
                    print_status(report)
            ...
            write_report(self.directory, self.state, final_status)
...
def main():
    ...
    parser.add_argument("--config", default=str(REPOSITORY / "configs/memetic_v2/office-0.2.0.json"))
