...
def controller(args):
    ...
    try:
        ...
        jobs = manifest['jobs']
        if len(jobs) != 11 or len({j['id'] for j in jobs}) != 11 or manifest['cover']['transports_checked'] != 10395:
            raise RuntimeError('Unexpected matching case cover')
        if any(j['group'] != 'totalizer' for j in jobs):
            raise RuntimeError('Only totalizer jobs permitted')
        queue = deque(jobs)
        guard = Guard(out)
        save(out/'config.json', {'version': VERSION, 'source': str(HERE),
             ...
             'total_allocated_wall_budget': 11*args.seconds, 'case_count': 11,
             ...
             'scope':'Single wave: eleven totalizer matching cases, bounded 34-hour scout; no certification.'})
        ...
        while queue or active:
            ...
            snapshot = dict(safety, phase='TOTALIZER_LONG_SEARCH', utc=datetime.datetime.now(datetime.timezone.utc).isoformat(),
                 ...
                 guard_write_retries=getattr(guard, 'write_retries', 0),
                 ETA_note='Single wave; time budget only, decision time unknown')
        ...
        if status!='GRAPH_FOUND_REQUIRES_REVIEW':
            status='TOTALIZER_LONG_COMPLETE_NO_CERTIFICATION'
...
def main():
    ...
    parser.add_argument('--seconds',type=int,default=122400,help='wall seconds per matching case; default and maximum 122400 (34 hours)')
    parser.add_argument('--workers',type=int,default=11)
    parser.add_argument('--controller',action='store_true',help=argparse.SUPPRESS)
    parser.add_argument('--out',help=argparse.SUPPRESS)
    args=parser.parse_args()
    if not 3<=args.seconds<=122400 or not 1<=args.workers<=11:
        parser.error('seconds: 3..122400; workers: 1..11')
    if args.controller:
        controller(args)
        return
    if not (ROOT/'setup.json').exists():
        parser.error('Run prepare_long.py first')
    ...
    for _ in range(30):
        if (out/'ready.json').exists():
            print(json.dumps({'status':'C2_TOTALIZER_LONG_READY','version':VERSION,'pid':proc.pid,'output':str(out),'proof_logging':False}),flush=True)
            return
        if proc.poll() is not None:
