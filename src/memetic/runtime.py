...
VERSION = "0.1.0"
SCHEMA = "conway99-memetic-checkpoint-v1"
DEFAULTS = {
    "version": VERSION,
    "master_seed": 2026091101,
    "workers": 3,
...
def fingerprint(repository, config):
    ...
    files = list((repository / "src/memetic").glob("*.py"))
    files += list((repository / "data/memetic/reference").glob("*"))
...
def make_status(state, config, resource_report, active_count):
    ...
    eta = None
    if len(samples) >= 8:
        eta = statistics.mean(samples[-128:]) * remaining / max(1, config["workers"])
    ...
    report = {"utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
              ...
              "batch_eta_seconds": eta, "eta_samples": len(samples),
              ...
              "eta_scope": "Batch completion from measured task wall times; no solution ETA"}
    return report
