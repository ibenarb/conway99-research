...
BASE = "15f6ce2550423ecfaf75847cfba824deecd563f3"
BRANCH = "memetic/office-v0.1.0-20260911"
VERSION = "0.1.0"
REQUIREMENTS = ("ortools==9.15.6755", "pynauty==2.8.8.1")
GENERATED = ("results/memetic/office_validation_0.1.0.json", "manifests/memetic/office_environment_0.1.0.json")
PREFIXES = ("src/memetic/", "configs/memetic/", "docs/memetic/", "data/memetic/", "results/memetic/", "manifests/memetic/")
...
def install_payload(repository, payload, accepted_previous=None):
    ...
    if replacing:
        run_root = repository.parent / "conway99_memetic_office_0.1.0/runs"
...
def make_environment(experiment, logfile):
    ...
    try:
        ...
        pass
    if not ready:
        print("Installiere die benötigten Python-Binärpakete in die Projektumgebung.", flush=True)
        execute([str(python), "-m", "pip", "install", "--only-binary=:all:", *REQUIREMENTS], logfile=logfile)
...
def validate_and_publish(repository, experiment, python, manifest, dependencies, logfile, publish):
    ...
    if not verified:
        ...
        execute([str(python), str(repository / "src/memetic/controls.py"), "--full", "--output", str(report_path)], logfile=logfile)
        report = json.loads(report_path.read_text())
    ...
    if changed:
        ...
        if diff:
            ...
            git(repository, "commit", "-m", "Add validated Office memetic pilot v0.1.0", logfile=logfile)
...
def launch(repository, experiment, python):
    ...
    with console.open("ab") as output:
        process = subprocess.Popen([str(python), str(repository / "src/memetic/runner.py"),
...
def main():
    ...
    experiment = workspace / "conway99_memetic_office_0.1.0"
...
if __name__ == "__main__":
    try:
        ...
        print("Kein Überspringen einer fehlgeschlagenen Kontrolle. Vollständiges Protokoll: ~/conway99_workspace/conway99_memetic_office_0.1.0/audits/setup.log", file=sys.stderr, flush=True)
        sys.exit(2)
