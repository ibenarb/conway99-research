...
def attempt(task):
    ...
    def consider(rows, value, source):
        ...
        if value > original_score or (best_data is not None and value > best_data["F"]):
            return
        ...
        data.update({"g6": encode_g6(rows), "canonical": identity,
                     "arm": arm, "family": parent_data["family"],
        ...
        if best is None or value < best_data["F"] or (value == best_data["F"] and novelty > previous):
            best, best_data = rows, data
            trace.append({"F": value, "phase": source, "evaluations": budget.evaluations})
    ...
    try:
        ...
        if kind == "crossover" and arm == "omega":
            ...
            budget.evaluate()
            consider(mixed, score(mixed), "crossover")
            current = mixed
            tabu.append(current)
        else:
    ...
    if best_data is not None:
        ...
        outcomes["strict_improvement" if best_data["F"] < original_score else "neutral_new"] += 1
